    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <nav className="bg-white border-b border-gray-200 sticky top-0 z-10 shadow-sm">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4 text-sm">
            ...
            <Link href="/login" className="text-gray-600 hover:text-gray-900 font-medium">Masuk</Link>
          </div>
        </div>
      </nav>
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600 mt-1">Informasi terbaru dari {profil?.namaDesaa ?? 'desa kami'}</p>
        </div>
        ...
        <form method="GET" className="mb-6 flex flex-wrap gap-2">
          <div className="relative flex-1 min-w-48">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-xl text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white shadow-sm"
            />
          </div>
          <select
            name="kategori"
            defaultValue={kategori}
            className="px-3 py-2.5 border border-gray-300 rounded-xl text-sm text-gray-900 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 shadow-sm"
          >
            ...
            <div className="space-y-3">
              {pinned.map((b) => (
                <Link
                  key={b.id}
                  href={`/berita/${b.slug}`}
                  className="flex items-start gap-4 bg-yellow-50 border border-yellow-300 rounded-2xl p-4 hover:bg-yellow-100 transition-colors"
                >
                  <div className="flex-1 min-w-0">
                    ...
                    {b.excerpt && (
                      <p className="text-xs text-gray-600 mt-0.5">{b.excerpt}</p>
                    )}
                    <p className="text-xs text-gray-500 mt-1">
                      {b.author.nama} · {formatTanggal(b.publishedAt ?? b.createdAt)}
                    </p>
                  </div>
                  {b.fotoUrl && (
                    <Image
            ...
            {beritaList.map((b) => (
              ...
              >
                {b.fotoUrl ? (
                  ...
                  <div className="w-full h-44 bg-linear-to-br from-blue-50 to-indigo-100 flex items-center justify-center text-5xl">
                    📰
                  </div>
                )}
                <div className="p-4">
                  ...
                  {b.excerpt && (
                    <p className="text-sm text-gray-600 mt-1 line-clamp-2">{b.excerpt}</p>
                  )}
