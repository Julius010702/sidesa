      ...
      <HeaderPage
        ...
        action={
          <div className="flex items-center gap-2">
            {/* Tombol Edit KK */}
            <Link
              href={`/admin/kartu-keluarga/${id}/edit`}
              className="inline-flex items-center gap-1.5 bg-amber-50 text-amber-700 border border-amber-200 px-3 py-2 rounded-xl text-sm font-semibold hover:bg-amber-100 transition-colors"
            >
            ...
            {/* Tombol Hapus KK */}
            <DeleteKKButton id={id} bisaDihapus={bisaDihapus} />

            {/* Tombol Tambah Anggota */}
            <Link
              href="/admin/penduduk/tambah"
              className="inline-flex items-center gap-1.5 bg-blue-600 text-white px-3 py-2 rounded-xl text-sm font-semibold hover:bg-blue-700 transition-colors shadow-sm"
            >
              <svg className="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            ...
            {kk.anggota.length === 0 ? (
              ...
              <div className="divide-y divide-gray-100">
                {kk.anggota.map((a: Anggota, i) => (
                  <div
                    key={a.id}
                    className="flex items-center gap-3 px-5 py-4 hover:bg-blue-50/40 transition-colors group"
                  >
                    {/* Nomor */}
                    <span className="text-xs font-bold text-gray-400 w-5 shrink-0 text-center">{i + 1}</span>

                    {/* Avatar */}
                    <Link href={`/admin/penduduk/${a.id}`} className="flex items-center gap-3 flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-1 flex-wrap">
                          ...
                          {a.pekerjaan && (
                            <>
                              ...
                              <span className="text-xs font-medium text-gray-600 truncate max-w-25">{a.pekerjaan}</span>
                            </>
                          )}
                        </div>
                      </div>
                    ...
                    {/* Aksi anggota */}
                    <AnggotaActions kkId={id} anggotaId={a.id} namaAnggota={a.nama} />
                  </div>
                ))}
              </div>
            )}
