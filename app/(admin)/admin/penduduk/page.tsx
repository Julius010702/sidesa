  ...
  const [data, total] = await Promise.all([
    prisma.penduduk.findMany({
      ...
      select: {
        ...
        tanggalLahir: true,
        pekerjaan: true,
        // ✅ fix: hubunganDenganKepala tidak ada di schema, dihapus
  ...
  return (
    <div>
      <HeaderPage
        ...
        breadcrumbs={[{ label: 'Dashboard', href: '/admin/dashboard' }, { label: 'Penduduk' }]}
        action={
          <Link
            href="/admin/penduduk/tambah"
            className="inline-flex items-center gap-2 bg-linear-to-r from-blue-600 to-indigo-600 text-white px-4 py-2 rounded-xl text-sm font-semibold hover:from-blue-700 hover:to-indigo-700 transition-all shadow-sm"
          >
      ...
      {data.length === 0 ? (
        <div className="bg-white border border-gray-100 rounded-2xl px-6 py-16 text-center">
          ...
          <Link
            href="/admin/penduduk/tambah"
            className="mt-4 inline-flex items-center gap-2 bg-blue-600 text-white px-5 py-2.5 rounded-xl text-sm font-semibold hover:bg-blue-700 transition-colors"
          >
        ...
        <>
          <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden">
            ...
            <div className="hidden md:grid grid-cols-12 gap-3 px-5 py-3 bg-gray-50 border-b border-gray-100 text-xs font-semibold text-gray-500 uppercase tracking-wider">
              <div className="col-span-4">Nama / NIK</div>
              <div className="col-span-2">L/P · Umur</div>
              <div className="col-span-2">Status Kawin</div>
                ...
                >
                  ...
                  <div className="md:col-span-4 flex items-center gap-3">
                    <div className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 font-bold text-sm ${
                      p.jenisKelamin === JenisKelamin.LAKI_LAKI  // ✅ fix
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-gray-400 font-mono">{p.nik}</p>
                    </div>
                  </div>
                  ...
                  <div className="md:col-span-2">
                    <p className="text-xs text-gray-700">
                      {p.statusKawin?.replace(/_/g, ' ') ?? '-'}  {/* ✅ fix */}
                    </p>
                  </div>

                  {/* Pekerjaan */}
                  <div className="md:col-span-2">
                    <p className="text-xs text-gray-600 truncate">{p.pekerjaan ?? '-'}</p>
                  </div>
