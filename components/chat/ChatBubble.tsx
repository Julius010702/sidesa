...
interface ChatBubbleProps {
  msg: Message
  isSelf: boolean // true = pesan dari diri sendiri (kanan)
  senderLabel?: string
}
      ...
      >
        ...
        {msg.tipe === 'image' && msg.mediaUrl && (
          <div className="flex flex-col gap-1.5">
            <a href={msg.mediaUrl} target="_blank" rel="noopener noreferrer">
              <div className="relative w-48 h-40 rounded-xl overflow-hidden">
                <Image
                  src={msg.mediaUrl}
                  alt="gambar"
                  fill
        ...
        {msg.tipe === 'voice' && msg.mediaUrl && (
          <div className="flex flex-col gap-1">
            <div className={`flex items-center gap-2 rounded-xl px-3 py-2 ${isSelf ? 'bg-blue-700' : 'bg-gray-200'}`}>
              ...
              <audio
                ...
                src={msg.mediaUrl}
                className="h-8 w-40"
                style={{ filter: isSelf ? 'invert(0.7)' : 'none' }}
              />
            </div>
          </div>
        )}

        <p className={`text-xs mt-1.5 ${isSelf ? 'text-blue-200' : 'text-gray-400'}`}>
